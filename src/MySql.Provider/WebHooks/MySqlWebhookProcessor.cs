...
{
    public class MySqlWebhookProcessor : BaseWebhookProcessor
    {
        public MySqlWebhookProcessor(ApplicationContext appContext)
            : base(appContext)
        {
        }
        ...
        {
            return webhookDefinition.ProviderId == MySqlConstants.ProviderId || base.Accept(webhookDefinition);
        }
                ...
                if (command.WebhookDefinition.ProviderDefinitionId != null)
                {
                    ...
                    if (jobDataCheck != null)
                    {
                        ...
                        IAgentJobProcessorArguments jobArgs = new DebugAgentJobProcessorArguments
                        {
                            ...
                            Job           = new AgentJob(Guid.NewGuid(), AgentJobPriority.Normal, "CluedIn" + MySqlConstants.ProviderName, ProcessingRestriction.Any, null, null)
                        };

                        var processorState = new AgentJobProcessorState<MySqlCrawlJobData>(jobArgs, AppContext)
                        {
